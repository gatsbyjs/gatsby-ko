...
종종, 여러분이 source 플러그인으로부터 받아오는 데이터의 형식은 여러분이 웹사이트를 만드는데 사용하기위해 원했던 형식이 아닐겁니다. filesystem source 플러그인은 여러분이 파일에 _대한_ 데이터를 쿼리해주지만 파일 _안에_ 있는 데이터는 어떻게 해야 할까요?
...
하지만 이것은 쉽게 고칠 수 있습니다. 어떠한 유형의 연결을 요청할 때는, 다양한 인자를 GraphQL 쿼리로 넘길 수 있습니다. 여러분은 노드를 `정렬`하고 `걸러낼 수` 있고, 몇개의 노드를 `건너뛸지` 설정할 수 있으며, 몇개의 노드를 검색할지 `제한`을 정할수도 있습니다. 이 강력한 연산자와 함께, 여러분은 여러분이 필요한 포맷으로 여러분이 필요한 데이터를 선택할 수 있습니다.
